...
export default function NewDeliveryForm({ operationId: propOperationId, isEditMode = false }) {
  ...
  // Fetch warehouses for internal transfers
  useEffect(() => {
    const fetchWarehouses = async () => {
      try {
        ...
        const data = await response.json();
        if (data.success) {
          setWarehouses([data.data]); // Currently only one warehouse
        }
      } catch (error) {
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-[#f5f5f7] pt-20 p-6 overflow-auto">
      ...
      <div className="mb-6 border-b border-gray-300 pb-6">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className={status === 'Waiting' ? 'text-purple-600 font-semibold' : 'text-gray-400'}>Waiting</span>
            <span className="text-gray-400">→</span>
        ...
        <div className="flex gap-3">
          ...
          {/* Waiting indicator */}
          {status === 'Waiting' && (
            <div className="px-6 py-2 rounded-lg border border-yellow-400 text-yellow-700 bg-yellow-100 font-semibold">
              ⏳ Waiting for Stock
            </div>
          )}
      ...
      <div className="border border-gray-300 rounded-lg p-6 bg-white shadow-sm">
        {/* Reference Number */}
        <div className="mb-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">{formData.reference || 'WH/OUT/0001'}</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            <label className="block text-gray-700 font-medium mb-2">Operation type *</label>
            <select
              name="operationType"
            ...
            >
              ...
              <option value="external">External Delivery</option>
              <option value="internal">Internal Transfer</option>
            </select>
            <p className="text-xs text-gray-500 mt-1">
              External: To customer/external location | Internal: Between warehouses
            </p>
          </div>

          {/* Show delivery address for external, warehouse selector for internal */}
          {formData.operationType === 'external' && (
            <div>
          ...
          {formData.operationType === 'internal' && (
            <div>
              <label className="block text-gray-700 font-medium mb-2">Destination Warehouse *</label>
              <select
            ...
            <table className="w-full">
              ...
              <tbody>
                {products.map((product) => (
                  <tr 
                    key={product.id} 
                    className={`border-b border-gray-200 ${!product.inStock && product.name ? 'bg-red-50' : 'bg-white'}`}
                  >
                    <td className="py-3 px-4">
                      <input
                        ...
                        className={`bg-transparent border-none text-gray-900 focus:outline-none w-full ${
                          !product.inStock && product.name ? 'text-red-600' : ''
                        }`}
                      ...
                      {stockCheckLoading && product.name && (
                        <p className="text-xs text-gray-500 mt-1">Checking stock...</p>
                      )}
                    </td>
                    <td className="py-3 px-4">
                      <input
